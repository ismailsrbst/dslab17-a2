...
public class MessageClientMailboxTest {
    ...
    public void setUp() throws Exception {
        ...
        Sockets.waitForSocket("localhost", mailboxConfig.getInt("dmep.tcp.port"), Constants.COMPONENT_STARTUP_WAIT);
    ...
    public void inbox_singleMail_showsAllInboxDataCorrectly() throws Exception {
        ...
        try (JunitSocketClient client = new JunitSocketClient(mailboxConfig.getInt("dmep.tcp.port"), err)) {
            err.checkThat("Expected mailbox server to implement DMEP2.0", client.read(), is("ok DMEP2.0"));
    ...
    public void inbox_mulipleMails_showsAllInboxDataCorrectly() throws Exception {
        ...
        try (JunitSocketClient client = new JunitSocketClient(mailboxConfig.getInt("dmep.tcp.port"), err)) {
            err.checkThat(client.read(), is("ok DMEP2.0"));
            client.sendAndVerify("begin", "ok");
