...
public class MailboxServerTest extends TestBase {
    ...
    public void runAndShutdownTransferServer_createsAndStopsTcpSocketCorrectly() throws Exception {
        ...
        try {
            LOG.info("Waiting for DMEP socket to open on port " + dmepPort);
            Sockets.waitForSocket("localhost", dmepPort, Constants.COMPONENT_STARTUP_WAIT);
        } catch (SocketTimeoutException e) {
            err.addError(new AssertionError("Expected a TCP server socket on port " + dmepPort, e));
        }
        ...
        err.checkThat("Expected tcp socket on port " + dmepPort + " to be closed after shutdown",
                Sockets.isServerSocketOpen(dmepPort), is(false));
